...
export default class CommentReader {
    public static async getComment(params: GetCommentParams): Promise<Comment> {
        const commentDb = await CommentRepository.findOne({ _id: params.commentId });
    ...
    public static async getCommentsForTask(params: GetAllCommentsParams): Promise<Comment[]> {
        const totalCommentsCount = await CommentRepository.countDocuments({ taskId: params.taskId });
        ...
        const commentsDb = await CommentRepository.find({ taskId: params.taskId })
